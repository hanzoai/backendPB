...
<Accordion single>
    ...
    <div class="content m-b-sm">
        <p>
            When HanzoBase is deployed on platforms like Fly or it is accessible through proxies such as
            NGINX, requests from different users will originate from the same IP address (the IP of the proxy
            connecting to your HanzoBase app).
        </p>
        <p>
            In this case to retrieve the actual user IP (used for rate limiting, logging, etc.) you need to
            properly configure your proxy and list below the trusted headers that HanzoBase could use to
            extract the user IP.
        </p>
        ...
        <ul class="m-t-0 txt-bold">
            ...
            <li>make sure that the HanzoBase server can be accessed only through the proxy</li>
        </ul>
        <p>You can clear the headers field if HanzoBase is not deployed behind a proxy.</p>
    </div>
